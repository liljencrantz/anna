...
#define DUCK_STRING_DEFAULT_ELEMENT_CAPACITY 6
...
static void duck_element_discard(duck_string_element_t *el)
{
    el->users--;
    if(el->users == 0) 
    {
...
static duck_string_ensure_element_capacity(duck_string_t *string, size_t count)
{
    ...
    {
	wprintf(L"Increasing capacity not yet supported, but we need to go from %d to %d. :-(\n");
	CRASH;
    }
}
...
void duck_string_clear(duck_string_t *dest)
{
  int i;
  for(i=0;i<dest->element_count; i++) {
      duck_element_discard(dest->element[i]);
  }
...
void duck_string_append(duck_string_t *dest, duck_string_t *src, size_t offset, size_t length)
{
  ...
  for(i=0;i<src->element_count; i++) {
    ...
    if(last_in_element >offset){
      ...
      {
	  for(j=1; j<src->element_count-i; j++)
	  {
