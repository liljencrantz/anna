...
//    anna_message(L"THREADSOR IS ALLOCATED\n\n\n");
}
...
//    anna_message(L"Pause thread\n");
	...
	pthread_mutex_unlock(&anna_alloc_mutex_gc);    
//	anna_message(L"Paused thread starting GC\n");
	anna_gc(context);
//	anna_message(L"Paused thread going back to waiting\n");
...
void anna_gc_first(anna_context_t *context)
{
    ...
    anna_alloc_work_count--;
    anna_message(L"Worker: Time for some GC (%d threads still running)\n", anna_alloc_work_count);    
...
//	alloc->work = 0;
	...
	anna_message(L"Worker %d: Wake up\n", anna_alloc_data()->idx);
...
static void anna_alloc_gc_wait_for_work_thread()
{
	...
	if(anna_alloc_work_count == 0 && anna_alloc_flag_gc == 1)
	{
	    ...
	    pthread_mutex_unlock(&anna_alloc_mutex_gc);
//	    anna_message(L"GC: Yay, time to work\n");
	    break;
	}
	pthread_cond_wait(&anna_alloc_cond_gc, &anna_alloc_mutex_gc);    
//	anna_message(L"GC: Wake up and smell the coffee\n");
    }
}
...
//    anna_message(L"GC: Start worker threads!\n");
    ...
    pthread_cond_broadcast(&anna_alloc_cond_gc);    
    pthread_mutex_unlock(&anna_alloc_mutex_gc);
}
